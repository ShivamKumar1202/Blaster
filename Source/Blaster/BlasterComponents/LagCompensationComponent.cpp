...
void ULagCompensationComponent::SaveFramePackage(FFramePackage& Package)
{
	...
	if (Character)
	{
		Package.Time = GetWorld()->GetTimeSeconds();	//Official Server time
...
FFramePackage ULagCompensationComponent::GetFrameToCheck(ABlasterCharacter* HitCharacter, float HitTime)
{
	...
	//Time flows from Tail to Head in the DLL, where Younger's time is greater than Older,i.e,
	// Younger was born after Older

	while (Older->GetValue().Time > HitTime) // is Older still younger than HitTime?
	{
		// March back until: OlderTime < HitTime < YoungerTime
		if (Older->GetNextNode() == nullptr) break;		//If Older is last node then next node will be nullptr so exit
