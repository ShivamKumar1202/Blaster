...
void UCombatComponent::SetSpeeds(float BaseSpeed)
{
...
void UCombatComponent::UpdateShotgunAmmoValues()
{
	...
	bCanFire = true;			//Interrupt reloading animation by shooting if your gun is not empty
...
void UCombatComponent::FireButtonPressed(bool bPressed)
{
	...
	if (bFireButtonPressed && EquippedWeapon)	// If you press Left Click while unequipped, bCanFire will always be false...so && EquippedWeapon needed
	{
		//UE_LOG(LogTemp, Warning, TEXT("Combat FBP"));
...
void UCombatComponent::FireProjctileWeapon()
{
	if (EquippedWeapon && Character)
	{
...
{
	if (Character->GetDisableGameplay())
	{
	...
	bool bScreenToWorld = UGameplayStatics::DeprojectScreenToWorld(		//Function to convert 2D space to 3D. CrosshairWorldPosition/Direction are outputs
...
void UCombatComponent::SetHUDCrosshairs(float DeltaTime)
{
	...
	if (Controller)
	{
		...
		if (HUD)
		{
			...
			{
				CrosshairInAirFactor = FMath::FInterpTo(CrosshairInAirFactor, 0.f, DeltaTime, 30.0f);
			}
...
void UCombatComponent::ServerLaunchGrenade_Implementation(const FVector_NetQuantize& Target)
{
	if (Character && GrenadeClass && Character->GetAttachedGrenade())
	{
		...
		const FVector StartingLocation = Character->GetMesh()->GetSocketLocation(FName("GrenadeSpawnSocket"));
...
void UCombatComponent::InitializeCarridAmmo()
{
...
void UCombatComponent::UpdateWeaponName()
{
	Controller = (Controller == nullptr) ? Cast<ABlasterPlayerController>(Character->Controller) : Controller;
	if (Controller)
	{
		//FString wn = UEnum::GetDisplayNameTextByValue(EquippedWeapon->GetWeaponType());
