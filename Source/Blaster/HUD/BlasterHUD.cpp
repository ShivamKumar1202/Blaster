...
void ABlasterHUD::DrawCrosshair(UTexture2D* Texture, FVector2D ViewportCenter, FVector2D Spread, FLinearColor CrosshairsColor)
{
	...
	DrawTexture(
		...
		TextureWidth,
		TextureHeight,
		0.f,	// Location in screen-space UV (0,0)
		0.f,	// 

		1.f,	// Size to draw in screen-space UV (1,1)
		1.f,	//
		CrosshairsColor
	);
}
				...
				if (Msg && Msg->AnnouncementBox)
				{
					...
					if (CanvasSlot)
					{
						...
						CanvasSlot->GetPosition().X,
							Position.Y - (CanvasSlot->GetSize().Y + 2.5f));		//Extra padding of 2.5 between each message
